...
function Chat({ params }) {
  ...
  useEffect(() => {
    setName(getCookie(`parentcraft_${lang}_chat_name`));
    fetchHistoryMsg();
    setMounted(true);
    return () => {
      setMounted(false);
  ...
  const setChatName = (e) => {
    if (e.target.checkValidity()) {
      e.preventDefault();
      let guest_name =
        "Guest" + " " + getCookie(`parentcraft_${lang}_uid`).slice(0, 5);
      if (chatAnony) {
        setName(guest_name);
      } else {
        ...
        setName(_name);
        setCookie(`parentcraft_${lang}_chat_name`, _name, {
...
function ChatLog({ chatLog }) {
  ...
  const uid = getCookie(`parentcraft_${lang}_uid`);
  return (
...
function ChatInput({ SendMessage, name }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setLoading(true);
      let res = await SaveMessage(lang, {
        date: today.toISOString().split("T")[0],
        userId: getCookie(`parentcraft_${lang}_uid`),
        name: name || "Anonymous",
        message: msg,
        timestamp: today,
      });
